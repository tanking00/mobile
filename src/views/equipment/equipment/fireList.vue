...
<template>
  <div class="container default-container-list">
    ...
    <div class="default-filterbar">
      <van-dropdown-menu>
        <van-dropdown-item v-model="type" title="设施类型" :options="typeOption" @change="onChange" />
        <van-dropdown-item v-model="status" title="设施状态" :options="statusOption" @change="onChange" />
          ...
          <div v-for="item in list" :key="item.id" class="list-item">
            ...
            >
              ...
              <div class="color-text-secondary clearfix">
                <span class="left">{{ `设施类型：${item.materialType}` }}</span>
    ...
    <SearchPanel v-model="showSearch" name="EquipmentEquipmentFireList" @search="onSearch"></SearchPanel>
  </div>
</template>
...
export default {
  ...
  async created() {
    this.typeOption = await getDropdownDataOfCode('materialType')
  },
  computed: {
    judgeHasClass() {
