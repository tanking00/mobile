...
import { getApplyDetails, applyApprovalSubmit } from '@/api/safetyReward'
...
export default {
      ...
      Dialog.confirm({
        ...
        .then(async () => {
          var params = {
            id: this.id,
            taskId: this.taskId,
            operation: operation, // 操作：1、提交2、同意3、退回4、转办5、结束
            processCode: this.processCode
          }
          params = Object.assign(params, data)
          console.log('params=', params)
          await applyApprovalSubmit(params)
          Toast('提交成功')
          this.$router.go(-1)
        })
        .catch(() => {
  ...
  computed: {
    ...
    // 是否在第一步审核时被退回
    isAuditBack() {
      // console.log('this.$route.query.isAuditBack=', this.$route.query.isAuditBack)
      return this.$route.query.isAuditBack
    }
  }
}
</script>
