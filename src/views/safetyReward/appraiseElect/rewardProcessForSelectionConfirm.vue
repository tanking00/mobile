  ...
  <div class="container submit-button-height">
    ...
    <div class="container-main padding-all">
      <van-collapse v-model="activeNames" class="default-collapse">
        ...
        <van-collapse-item name="2" title="评选信息">
          <template #right-icon>
          ...
          <van-collapse v-model="infoActives" class="embedded-collapse" :border="false">
            ...
            >
              ...
              <div class="btns">
                ...
                >
                  <van-button plain type="info" class="btn">查看</van-button>
                </router-link>
                <van-button plain type="info" class="btn" @click="goEdit(item)">奖项编辑</van-button>
              </div>
            </van-collapse-item>
          </van-collapse>
        </van-collapse-item>
    ...
    <SubmitButton>
      ...
      <SubmitButtonItem @click="submitButtonClik">提交</SubmitButtonItem>
    </SubmitButton>
...
import { AwardEditingPanel } from './components'
import { getAppraiseDetails, appraiseAffirmSubmit } from '@/api/safetyReward'
...
export default {
  ...
  components: { ApplyTopInfoView, AwardEditingPanel },
  data() {
    return {
      showEditView: false,
      activeNames: ['1', '2'],
  ...
  methods: {
    ...
    submitButtonClik() {
      for (var i = 0; i < this.details.submissionDetail.length; i++) {
        const item = this.details.submissionDetail[i]
    ...
    approval(tips, operation, data) {
      Dialog.confirm({
        ...
        .then(async () => {
          var submissions = this.details.submissionDetail.map(item => {
            return {
              id: item.id,
              award: item.award
            }
          ...
          await appraiseAffirmSubmit(params)
          Toast('提交成功')
          this.$router.go(-1)
        })
        .catch(() => {
...
//  ::v-deep  .myline .default-collapse .van-collapse-item .van-collapse-item__content .van-divider {
