<!-- 违规处罚-会签(行政人事总监/安委会常任副主任/战略副总裁会签)-->
<template>
  <div class="container submit-button-height">
    ...
    <SubmitButton>
      <SubmitButtonItem @click="showPanel1 = true">退回</SubmitButtonItem>
...
import { getPunishInfoDetails, punishInfoCountersign } from '@/api/safetyReward'
    ...
    approval(tips, operation, data) {
      Dialog.confirm({
        ...
        .then(async () => {
          ...
          await punishInfoCountersign(params)
          Toast('提交成功')
          this.$router.go(-1)
        })
        .catch(() => {
