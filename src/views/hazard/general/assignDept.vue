...
<script>
import { getHazardGeneralDetails, commitAssignDep, getUserOrg } from '@/api/hazard'
    ...
    getDetails() {
      ...
      getHazardGeneralDetails(req)
        .then(res => {
          ...
          this.hazardCommit.classifyId = this.hazardDetailsBO.classifyId
          this.hazardCommit.levelId = this.hazardDetailsBO.levelId
          this.hazardCommit.narrate = this.hazardDetailsBO.narrate
          this.hazardCommit.source = this.hazardDetailsBO.source
          this.hazardCommit.subclassId = this.hazardDetailsBO.subclassId
          this.hazardCommit.projectName = this.hazardDetailsBO.projectName
          this.hazardCommit.projectNo = this.hazardDetailsBO.projectNo
          this.hazardCommit.isMajor = '0'
          let files = ''
          if (this.hazardDetailsBO.fileList) {
            this.hazardDetailsBO.fileList.forEach((item, index) => {
              if (index !== 0) {
                files += ',' + item.id
              } else {
                files += item.id
      ...
      Dialog.confirm({
        ...
        .then(() => {
          ...
          const req = {
            ...
            hazardCommit: {
              classifyId: this.hazardDetailsBO.classifyId,
              levelId: this.hazardDetailsBO.levelId,
              narrate: this.hazardDetailsBO.narrate,
              source: this.hazardDetailsBO.source,
              subclassId: this.hazardDetailsBO.subclassId,
              projectName: this.hazardDetailsBO.projectName,
              projectNo: this.hazardDetailsBO.projectNo,
              files: this.oldFiles
            }
          }
          commitAssignDep(req)
