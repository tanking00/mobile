    ...
    <SubmitPanel v-model="showPanel3" title="审批意见" buttonText="确认提交" :isLarge="true" @submit="onClickForm3">
      <van-form ref="form3" @failed="onFailedForm3" @submit="onSubmitForm3" :show-error-message="false">
        <van-field
          ...
          :rules="[{ required: true, message: '请选择整改责任部门' }]"
        />
        <van-field
...
import { BasicInfo, ReportInfo, ApprovalRecordInfo } from '../components'
...
export default {
  ...
  components: { BasicInfo, ReportInfo, ApprovalRecordInfo },
  data() {
    return {
      activeNames: ['2'],
      stepActive: 1,
    ...
    getDetails() {
      ...
      getHazardGeneralDetails(req)
        .then(res => {
          ...
          if (data.hazardDetailsVo.dutyDepts.length > 1) {
            this.isCanDelete = true
          }

          data.hazardDetailsVo.dutyDepts.forEach(item => {
            if (item.hazardSubId === this.hazardSubId) {
              this.form3 = item
            }
          })
          const stepInfo1 = []
