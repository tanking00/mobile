          ...
          <div>
            <div class="dutyDept-wrapper" v-for="(item, index) in dutyDepts" :key="item.id">
              ...
              <div class="item-center">
                ...
                <div class="text">{{ item.deptManager }}</div>
                <div class="label">督办总监</div>
                <div class="text">{{ item.deptSupervise }}</div>
              </div>
            </div>
          </div>
        </van-collapse-item>
...
<script>
import { getHazardById, commitChiefSecurityOfficerConfirm } from '@/api/hazard'
    ...
    onSubmitForm1() {
      Dialog.confirm({
        ...
        .then(() => {
          ...
          commitChiefSecurityOfficerConfirm(req)
            .then(res => {
