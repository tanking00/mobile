  ...
  <div class="container" :class="{ 'submit-button-height': taskId, 'container-supports': !taskId }">
    ...
    <div class="container-main padding-all">
      <van-collapse v-model="activeNames" class="default-collapse">
        ...
        <van-collapse-item v-if="recordList && recordList.length > 0" name="6" title="历史审批信息">
          <template #right-icon>
    ...
    <SubmitPanel v-model="showPanel3" title="审批意见" buttonText="确认提交" @submit="onClickForm3">
      <van-form ref="form3" @failed="onFailedForm3" @submit="onSubmitForm3" :show-error-message="false">
        <van-field
          v-model="form3.acceptor"
          name="整改验收人"
          label="整改验收人"
          placeholder="请选择"
          required
          ...
          size="large"
          @click="showPeopleSearchPanel = true"
          :rules="[{ required: true, message: '请选择整改验收人' }]"
        />
...
<script>
import { getHazardMajorDetails, commitSafetyDepartmentAudit } from '@/api/hazard'
...
export default {
  ...
  data() {
    return {
      ...
      steps: [
        ...
        { title: '制定整改方案（已完成）', time: '' },
        { title: '督办总监审核（已完成）', time: '' },
        { title: '安全部门审核（进行中）', time: '' },
        { title: '首席安全官审核', time: '' },
        { title: '隐患整改执行', time: '' },
        { title: '督办总监确认', time: '' },
        { title: '整改验收', time: '' },
  ...
  methods: {
    ...
    onSelect(data) {
      ...
      this.form3.acceptor = data.name
      this.form3.acceptorId = data.id
    },
    // 退回
    onClickForm1() {
    ...
    onSubmitForm1() {
      Dialog.confirm({
        ...
        .then(() => {
          ...
          commitSafetyDepartmentAudit(req)
            .then(res => {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          ...
          const req = {
            ...
            acceptor: this.form3.acceptor,
            acceptorId: this.form3.acceptorId,
            nextUserId: '',
            nextUserName: ''
          }
          commitSafetyDepartmentAudit(req)
