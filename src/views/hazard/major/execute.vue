  ...
  <div class="container" :class="{ 'submit-button-height': taskId, 'container-supports': !taskId }">
    ...
    <div class="container-main padding-all">
      <van-collapse v-model="activeNames" class="default-collapse">
        ...
        <van-collapse-item v-if="recordList && recordList.length > 0" name="7" title="历史审批信息">
          <template #right-icon>
    ...
    <SubmitPanel v-model="showPanel3" title="审批意见" buttonText="确认提交" :isLarge="true" @submit="onClickForm3">
      <van-form ref="form3" @failed="onFailedForm3" @submit="onSubmitForm3" :show-error-message="false">
        <div class="form-label-required">整改结果</div>
        <van-field
          v-model="form3.abarbeitungResults"
          rows="4"
          type="textarea"
          maxlength="1000"
          placeholder="请填写整改结果"
...
<script>
import { getHazardMajorDetails, commitImplementationRectification } from '@/api/hazard'
  ...
  data() {
    return {
      ...
      steps: [
        ...
        { title: '安全部门审核（已完成）', time: '' },
        { title: '首席安全官审核（已完成）', time: '' },
        { title: '隐患整改执行（进行中）', time: '' },
        { title: '督办总监确认', time: '' },
        { title: '整改验收', time: '' },
        { title: '首席安全官确认', time: '' },
        { title: '流程结束', time: '' }
    ...
    onSubmitForm1() {
      Dialog.confirm({
        ...
        .then(() => {
          ...
          commitImplementationRectification(req)
            .then(res => {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          ...
          const req = {
            ...
            nextUserId: '',
            nextUserName: '',
            abarbeitungResults: this.form3.abarbeitungResults,
