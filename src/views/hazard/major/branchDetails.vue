  ...
  <div class="container container-supports">
    ...
    <div class="container-main padding-all">
      <van-collapse v-model="activeNames" class="default-collapse">
        ...
        <van-collapse-item v-if="nodeIndex >= 6" name="6" title="安全部门审核">
          <template #right-icon>
        ...
        <van-collapse-item v-if="nodeIndex >= 8" name="7" title="隐患整改执行">
          <template #right-icon>
            <CollapseIcon :isOpen="activeNames.indexOf('7') >= 0"></CollapseIcon>
          </template>
          <ExecuteInfo :details="details"></ExecuteInfo>
        </van-collapse-item>
        <van-collapse-item v-if="nodeIndex >= 10" name="8" title="整改验收">
          <template #right-icon>
...
<script>
import { getHazardMajorDetails } from '@/api/hazard'
      ...
      getHazardMajorDetails(this.hazardId, this.hazardSubId)
        .then(res => {
          ...
          data.hazardRecords.forEach(item => {
            ...
            if (item.nodeName === '验收人验收') {
              stepInfo10.push(item)
            }
            if (item.nodeName === '首席安全官确认') {
              stepInfo11.push(item)
            }
          })
          switch (data.nodeIndex) {
            ...
            case 10:
              ...
              this.steps[8].title = '督办总监确认（已完成）'
              this.steps[8].time = stepInfo9[0] ? formatTimeStamp(stepInfo9[0].createdTime, 6) : ''
              this.steps[9].title = '整改验收（已完成）'
              this.steps[9].time = stepInfo10[0] ? formatTimeStamp(stepInfo10[0].createdTime, 6) : ''
              this.steps[10].title = '首席安全官确认（进行中）'
              break
            case 11:
              this.activeNames.push('9')
              this.stepActive = 12
              this.steps[0].title = '隐患提报（已完成）'
              this.steps[0].time = stepInfo1[0] ? formatTimeStamp(stepInfo1[0].createdTime, 6) : ''
              this.steps[1].title = '提报部门审核（已完成）'
              ...
              this.steps[9].title = '整改验收（已完成）'
              this.steps[9].time = stepInfo10[0] ? formatTimeStamp(stepInfo10[0].createdTime, 6) : ''
              this.steps[10].title = '首席安全官确认（已完成）'
              this.steps[10].time = stepInfo11[0] ? formatTimeStamp(stepInfo11[0].createdTime, 6) : ''
              break
          }
        })
        .catch(err => {
